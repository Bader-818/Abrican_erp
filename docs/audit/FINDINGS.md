...
| F-011 | INFO | FIXED 2026-07-08 | D6 Approvals | Timesheets and daily reports had **no self-approval block** — a user holding both `*.manage` and `*.approve` could approve their own submission (expenses already blocked this, INV-D8-3). | Owner decided to enforce separation of duties everywhere: `assertNotSelf` extended to timesheets and daily reports (approve **and** reject → 403 for the creator). Unit tests prove the creator is blocked and a different approver succeeds. |
| F-012 | INFO | FIXED 2026-07-08 | D7 Numbering | Deleting DRAFT invoices left **gaps in the invoice number sequence** (numbers were assigned at creation). ZATCA expects sequential tax-invoice numbering. | `Invoice.invoiceNumber` is now nullable: drafts carry only their internal id, and the official sequential number is assigned **inside the issue transaction** (max-derived, unique-constraint + retry). Issued invoices cannot be hard-deleted (409 pointing to the credit-note path); pre-issue cancel unchanged. Migration `20260708081521_invoice_number_at_issue`. E2E proves create→delete draft→issue yields consecutive numbers with no gap. |

### Verdict
F-007…F-010 fixed with regression tests. F-011/F-012 (design decisions) resolved and fixed
2026-07-08 — **all findings in this register are now closed**. Suites green after the fixes
(212 unit / 34 e2e / 27 frontend).
